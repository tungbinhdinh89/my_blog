...
export default function DashPosts() {
  ...
  const handleShowMore = async () => {
    const startIndex = userPosts.length; // continue load rest post, start from the previous post
    try {
      const res = await fetch(
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setUserPosts((prev) => {
          return prev.filter((post) => post._id !== postIdToDelete); // keep posts with different ids than the deleted post
        });
      }
    } catch (error) {
      console.log(error.message);
